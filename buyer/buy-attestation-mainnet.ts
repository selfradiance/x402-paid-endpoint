...
const GATED_URL = "https://web-attestation.selfradiance.workers.dev/attest?url=https://example.com";
const SPEND_RECEIPT_CONFIG_ROOT = "buyer/.spend-receipt-mainnet-attestation";
const SPEND_RECEIPT_CONFIG_DIR = join(SPEND_RECEIPT_CONFIG_ROOT, "x402-spend-receipt");
const EXPECTED_NETWORK = "eip155:8453";
const EXPECTED_USDC_ASSET = "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913";
...
function selectRequirement(_x402Version: number, accepts: PaymentRequirements[]): PaymentRequirements {
  const requirement = accepts.find(item => item.scheme === "exact" && item.network === EXPECTED_NETWORK);
  assert(requirement, "No exact " + EXPECTED_NETWORK + " payment requirement found in PAYMENT-REQUIRED header.");
  assert(
    requirement.asset.toLowerCase() === EXPECTED_USDC_ASSET.toLowerCase(),
    "Expected USDC asset " + EXPECTED_USDC_ASSET + ", got " + requirement.asset + ".",
  );
  return requirement;
}
...
registerExactEvmScheme(coreClient, { signer, networks: [EXPECTED_NETWORK] });
...
console.log("Decoded live payment requirements:");
...
try {
  ...
  if (policyResult.decision === "DENY") {
    ...
    console.log("DRY_RUN=1: exiting before payment signing.");
  } else {
    ...
    if (paidResponse.status !== 200) {
      const errorBody = await paidResponse.clone().text();
      console.log("Paid retry error body:");
