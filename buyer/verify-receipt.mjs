...
const keyDoc = await (await fetch("https://selfradiance.github.io/.well-known/issuer-key.json")).json();
console.log("Published key doc:", JSON.stringify(keyDoc));

const rawKey = Buffer.from(keyDoc.publicKeyBase64, "base64");
...
const message = Buffer.from(canonicalStringify(body.receipt), "utf8");
...
console.log("Inline key matches published key:", (keyDoc.publicKeyBase64) === body.verification_key);
